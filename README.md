...
2021.对于"中断处理结束"、"进程阻塞"、"进程执行结束"和"进程时间片用完"操作，都会引起进程调度程序的执行。此外，常会考察进程状态的转换，主动出让`CPU`或分时系统``CPU`时间片用完，线程会由执行态转换为就绪态。用户级线程完全由用户管理，多线程模型的多对一模式，当一个线程使用内核服务时被阻塞，则整个进程都会被阻塞。
